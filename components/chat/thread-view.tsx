...
export function ThreadView({ parentMessage, onReply, onReactionAdd, onReactionRemove, disabled }: ThreadViewProps) {
      ...
      <DialogTrigger asChild>
        <Button variant="ghost" size="sm" className="text-xs text-muted-foreground hover:text-foreground">
          ...
          {parentMessage.replies.length > 0
            ? `${parentMessage.replies.length} ${parentMessage.replies.length === 1 ? "reply" : "replies"}`
            : "Reply"}
        </Button>
      </DialogTrigger>
      ...
      <DialogContent className="max-w-2xl max-h-[80vh] flex flex-col">
        ...
        <div className="flex-1 flex flex-col min-h-0">
          ...
          <div className="border-t mt-4 pt-4">
            <MessageInput onSendMessage={handleReply} disabled={disabled} placeholder="Reply to thread..." />
          </div>
        </div>
      </DialogContent>
    </Dialog>
